...
//  check for previous submittals/approvals
     	...
     	def wfMsgRow = trmUtilities.executeQuery("SELECT Approval_Cd, Approval_Req_User, Approval_Req_Dttm, Approver_User, Approval_Dttm FROM TRM_META_DB.EX_EXTRACT_FORMAT WHERE EXTRACT_FORMAT_ID = '" + trmComponent.id + "'");         
...
// if submitted, send rejection
	if (approvalCd == 1) {
		...
		def wfMsgRow2 = trmUtilities.executeQuery("UPDATE TRM_META_DB.EX_EXTRACT_FORMAT SET Approval_Cd = 3, " +
...
//			trmComponent.attributes.setValue('Approval Status', 3) ;
	     		...
	     		def text = "Extract Format " + trmComponent.link + " has been rejected by " + trmCurrentUser.displayname + ".<BR><BR>";
	     		def subject = trmComponent.componentType + " " + trmComponent.name  + " has been rejected.";
// 	   	        +  currentTimestamp + " " + trmCurrentUser.displayname + ".\n\n";
			...
			trmUtilities.sendEmail(fromEmail,toEmail,text,subject);
		     ...
		     trmInfoBar.addInfoMessage("Extract Format rejection has been sent to " + submitterName + ".");
		} else {
