...
FOR /F "usebackq delims=" %%i IN ("%TRM_MANIFEST_FILE%") DO (
   echo off
   find /v "" "%%i" > ~Temp%TRM_TPLT%%CURRTM%.csv      
   for /f "tokens=1,* delims=[]" %%a in (~Temp%TRM_TPLT%%CURRTM%.csv) do set Last=%%a
   type "%%i" | find /v "%Last%" > newfile%TRM_TPLT%%CURRTM%.csv
   echo on
   copy /B newfile%TRM_TPLT%%CURRTM%.csv "\\%%~pi%%~ni%CURRTM%.csv"

   del /Q ~Temp%TRM_TPLT%%CURRTM%.csv
   del /Q newfile%TRM_TPLT%%CURRTM%.csv
   del /Q "%%i"
)
